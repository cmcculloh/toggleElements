...
;(function($){
	 ...
	 * > }
	 */
	jQuery.toggleSelects.beforeToggle = function(){
	...
	var doToggle = function(opts){
		...
		var $elms = $(opts.parentSelector + " " + opts.elementType + opts.elementSelector);
		
		//if not byElement or byArea
		if(!opts.byElement || !opts.byArea.areaX){
			if(opts.toggleTo === "autoSelect"){
				$elms.find(':visible').hide();
				$elms.find(':hidden').show();
